...
function mapObra(r){const opus=norm(get(r,['Nr OPUS','Nº OPUS','OPUS','Nr. OPUS']));const contrato=norm(get(r,['Contrato','Nº Contrato','Nr Contrato']));return {opus,contrato,rm:norm(get(r,['RM','Região Militar'])),contratante:norm(get(r,['Contratante','CRO','GPT'])),om_beneficiada:norm(get(r,['OM Beneficiada','OM'])),descricao:norm(get(r,['Descrição','Concepção do Objeto','Objeto'])),nome_obra:norm(get(r,['Nome da Obra','Obra'])),empresa:norm(get(r,['Empresa','Contratada'])),valor_atual:num(get(r,['Valor Atual','Valor Total','VALOR TOTAL'])),total_ne:num(get(r,['Total NE','Empenho','EMPENHO'])),total_nf:num(get(r,['Total NF','Executado','EXECUTADO'])),percentual_medido:num(get(r,['% Medido','Percentual Medido'])),percentual_estimado:num(get(r,['% Estimado','Percentual Estimado'])),dados:r}}
