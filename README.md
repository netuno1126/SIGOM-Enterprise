...
# Fase 12.34.5 — Integração física dos novos módulos

## Correção

A 12.34.4 dependia de injeção dinâmica de botões. Na 12.34.5 os acessos foram gravados
diretamente nos HTML principais, portanto continuam visíveis mesmo que o script auxiliar
de contador não carregue.

## Dashboard SIGOM

O menu principal contém fisicamente:
...
A barra de abas contém fisicamente:

- 👁 Visão do Diretor
- 🔔 Medições

O menu de Objetivos também contém:
...
`public/app/fase-12-34-5-contadores.js` consulta `alertas_medicao` e acrescenta a quantidade
de registros não lidos aos botões. O contador é apenas um aprimoramento; a navegação
não depende dele.
...
- `public/app/sugestoes.html`

## Supabase

A 12.34.5 não exige nova alteração estrutural no banco operacional. Ela utiliza as
estruturas já criadas nas fases anteriores, inclusive `alertas_medicao`,
`objetivo5_lista_referencia` e `sugestoes_melhoria`.

## Publicação

Copie o conteúdo do pacote sobre a raiz do repositório e faça somente:

```bash
git add .
git commit -m "fix: integrar fisicamente alertas, Visão do Diretor e Caixa de Sugestões na Fase 12.34.5"
git push origin main
```

Não execute Python e não é necessário editar manualmente os HTML.
